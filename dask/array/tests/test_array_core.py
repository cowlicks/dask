...
@pytest.mark.skipif(int(np.__version__[2]) < 8,
                    reason="Scalar indices not supported until numpy 1.8")
def test_take():
    x = np.arange(400).reshape((20, 20))
...
@pytest.mark.skipif(int(np.__version__[2]) < 8,
                    reason=("isclose not added until numpy 1.7, and is broken "
                            "for some " "reason in numpy 1.8"))
def test_isclose():
...
@pytest.mark.skipif(int(np.__version__[2]) < 7,
                    reason="multiple axis arguments in sum not supported until"
                           " numpy 1.7")
...
@pytest.mark.skipif(int(np.__version__[2]) < 7,
                    reason=("multiple axis arguments in sum not supported "
                            "until numpy 1.7"))
def test_coarsen():
