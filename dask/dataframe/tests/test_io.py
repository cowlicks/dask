...
def test_hdf_globbing():
    ...
    try:
        ...
        print("df.to_hdf(os.path.join(tdir, 'two.h5'), '/bar/data', format='table'")
        df.to_hdf(os.path.join(tdir, 'two.h5'), '/bar/data', format='table')
        print("df.to_hdf(os.path.join(tdir, 'two.h5'), '/foo/data', format='table'")
        df.to_hdf(os.path.join(tdir, 'two.h5'), '/foo/data', format='table')

        print("res = dd.read_hdf(os.path.join(tdir, 'one.h5'), '/*/data', chunksize=2)")
        res = dd.read_hdf(os.path.join(tdir, 'one.h5'), '/*/data',
                          chunksize=2)
        assert res.npartitions == 2
        print("tm.assert_frame_equal(res.compute(), df)")
        tm.assert_frame_equal(res.compute(), df)

        print("res = dd.read_hdf(os.path.join(tdir, 'one.h5'), '/*/data', chunksize=2, start=1, stop=3)")
        res = dd.read_hdf(os.path.join(tdir, 'one.h5'), '/*/data',
                          chunksize=2, start=1, stop=3)
        print("expected = pd.read_hdf(os.path.join(tdir, 'one.h5'), '/foo/data', start=1, stop=3)")
        expected = pd.read_hdf(os.path.join(tdir, 'one.h5'), '/foo/data',
                               start=1, stop=3)
        print("tm.assert_frame_equal(res.compute(), expected)")
        tm.assert_frame_equal(res.compute(), expected)

        print("res = dd.read_hdf(os.path.join(tdir, 'two.h5'), '/*/data', chunksize=2)")
        res = dd.read_hdf(os.path.join(tdir, 'two.h5'), '/*/data', chunksize=2)
        assert res.npartitions == 2 + 2
        print("tm.assert_frame_equal(res.compute(), pd.concat([df] * 2))")
        tm.assert_frame_equal(res.compute(), pd.concat([df] * 2))

        print("res = dd.read_hdf(os.path.join(tdir, '*.h5'), '/foo/data', chunksize=2)")
        res = dd.read_hdf(os.path.join(tdir, '*.h5'), '/foo/data', chunksize=2)
        assert res.npartitions == 2 + 2
        print("tm.assert_frame_equal(res.compute(), pd.concat([df] * 2))")
        tm.assert_frame_equal(res.compute(), pd.concat([df] * 2))

        print("res = dd.read_hdf(os.path.join(tdir, '*.h5'), '/*/data', chunksize=2)")
